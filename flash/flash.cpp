...
int flashRead3(flashaddr_t address, char* buffer, size_t size)
{
...
int programaHalfWord(uint16_t *direccion, uint16_t valor)
{
    char buffer[4];
    memcpy(buffer, &valor, 2);
    return flashWrite((flashaddr_t)direccion, (const char*) buffer, 2);
}
